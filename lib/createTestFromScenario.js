  ...
  after(function() {
    cy.then(() => testState.onFinishTest()).then(() => {
      if (window.cucumberJson.generate) {
        // send the testState to the node side cucumber JsonFormatter
        // and write out the returned cucumber.json file
        cy.task("generateCucumberJson", testState, { log: false }).then(
          writeCucumberJsonFile
        );
      }
    });
  });
