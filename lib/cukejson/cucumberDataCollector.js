  ...
  constructor(realUri, cucumberJson) {
    ...
    this.onFinishTest = () => {
      if (cucumberJson.generate) {
        ...
        const output = JSON.parse(this.getOutput());
        const outputFolder =
          cucumberJson.outputFolder || "cypress/cucumber-json";
        const outputPrefix = cucumberJson.filePrefix || "cucumber-";
        const name = output[0] ? makeName(output[0].uri) : "empty";
        const outFile = `${outputFolder}/${outputPrefix}${name}.json`;
        cy.writeFile(outFile, output);
      }
    };
